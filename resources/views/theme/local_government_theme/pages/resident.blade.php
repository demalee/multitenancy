@extends('theme.local_government_theme.layouts.apps')
@section('content')
			<!-- SAB BANNER START-->
			<div class="sab_banner overlay">
				<div class="container">
					<div class="sab_banner_text">
						<h2 contenteditable="true">Resident</h2>
						<ul class="breadcrumb">
						  <li class="breadcrumb-item"><a href="#" contenteditable="true">Home</a></li>
						  <li class="breadcrumb-item active" contenteditable="true">Resident</li>
						</ul>
					</div>
				</div>
			</div>
			<!-- SAB BANNER END-->

			<!-- CITY SERVICES2 WRAP START-->
			<div class="city_services2_wrap">
				<div class="container">
					<div class="row">
						<div class="col-md-4 col-sm-6">
							<div class="city_business_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/business_fig.jpg')}}" alt="">
									<div class="city_service2_list">
										<span><i class="fa icon-classroom"></i></span>
										<div class="city_service2_caption">
											<h5 contenteditable="true">Licences and Permit</h5>
										</div>
									</div>
								</figure>
								<div class="city_business_list">
									<ul class="city_busine_detail">
										<li><a href="#" contenteditable="true"><i class="fa fa-star-o"></i> Taxi licences</a></li>
										<li><a href="#" contenteditable="true"><i class="fa fa-star-o"></i>Entertainment & alcohol Permit</a></li>
										<li><a href="#" contenteditable="true"><i class="fa fa-star-o"></i>Business and street trading</a></li>
										<li><a href="#" contenteditable="true"><i class="fa fa-star-o"></i>Child work & performance</a></li>
									</ul>
									<a class="see_more_btn" href="#">See All Permit <i class="fa icon-next-1"></i></a>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6">
							<div class="city_business_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/business_fig1.jpg')}}" alt="">
									<div class="city_service2_list">
										<span><i class="fa icon-bag"></i></span>
										<div class="city_service2_caption">
											<h5 contenteditable="true">Doing Business</h5>
										</div>
									</div>
								</figure>
								<div class="city_business_list">
									<ul class="city_busine_detail">
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Taxi licences</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Entertainment & alcohol Permit</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Business and street trading</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Child work & performance</a></li>
									</ul>
									<a class="see_more_btn" href="#">See All Permit <i class="fa icon-next-1"></i></a>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6">
							<div class="city_business_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/business_fig2.jpg')}}" alt="">
									<div class="city_service2_list">
										<span><i class="fa icon-home"></i></span>
										<div class="city_service2_caption">
											<h5 contenteditable="true">Land and Property</h5>
										</div>
									</div>
								</figure>
								<div class="city_business_list">
									<ul class="city_busine_detail">
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Taxi licences</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Entertainment & alcohol Permit</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Business and street trading</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Child work & performance</a></li>
									</ul>
									<a class="see_more_btn" href="#">See All Permit <i class="fa icon-next-1"></i></a>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6">
							<div class="city_business_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/business_fig3.jpg')}}" alt="">
									<div class="city_service2_list">
										<span><i class="fa icon-gear"></i></span>
										<div class="city_service2_caption">
											<h5 contenteditable="true">Development Permit</h5>
										</div>
									</div>
								</figure>
								<div class="city_business_list">
									<ul class="city_busine_detail">
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Taxi licences</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Entertainment & alcohol Permit</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Business and street trading</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Child work & performance</a></li>
									</ul>
									<a class="see_more_btn" href="#">See All Permit <i class="fa icon-next-1"></i></a>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6">
							<div class="city_business_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/business_fig4.jpg')}}" alt="">
									<div class="city_service2_list">
										<span><i class="fa icon-bid"></i></span>
										<div class="city_service2_caption">
											<h5 contenteditable="true">Tenders & Bids</h5>
										</div>
									</div>
								</figure>
								<div class="city_business_list">
									<ul class="city_busine_detail">
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Taxi licences</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Entertainment & alcohol Permit</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Business and street trading</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Child work & performance</a></li>
									</ul>
									<a class="see_more_btn" href="#">See All Permit <i class="fa icon-next-1"></i></a>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6">
							<div class="city_business_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/business_fig5.jpg')}}" alt="">
									<div class="city_service2_list">
										<span><i class="fa icon-classroom"></i></span>
										<div class="city_service2_caption">
											<h5 contenteditable="true">Trading Standards</h5>
										</div>
									</div>
								</figure>
								<div class="city_business_list">
									<ul class="city_busine_detail">
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Taxi licences</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Entertainment & alcohol Permit</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Business and street trading</a></li>
										<li contenteditable="true"><a href="#"><i class="fa fa-star-o"></i>Child work & performance</a></li>
									</ul>
									<a class="see_more_btn" href="#">See All Permit <i class="fa icon-next-1"></i></a>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
			<!-- CITY SERVICES2 WRAP END-->

			<!-- CITY VISIT WRAP START-->
			<div class="city_visit_wrap">
				<div class="container-fluid">
					<div class="row">
						<div class="col-md-6 col-sm-6">
							<div class="city_visit_fig">
								<figure class="box">
									<div class="box-layer layer-1"></div>
									<div class="box-layer layer-2"></div>
									<div class="box-layer layer-3"></div>
									<img src="{{asset('theme/local/extra-images/visit-fig.jpg')}}" alt="">
								</figure>
							</div>
						</div>
						<div class="col-md-6 col-sm-6">
							<div class="city_visit_text">
								<h3 contenteditable="true">What Are You <span>Wondering</span></h3>
								<h3 contenteditable="true">Are You A <span>Resident!</span></h3>
								<p contenteditable="true"><i>Proin gravida nibh vel velit auctor aliquet Aenean <br>sollicitudin, </i></p>
								<p contenteditable="true">Lorem quis bibendum auctor, nisi elit consequat ipsum, nec <br>sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate<br> cursus a sit amet mauris. </p>
								<a class="theam_btn border-color color" href="#" tabindex="0">Explore More</a>
							</div>
						</div>
						<div class="col-md-6 col-sm-6">
							<div class="city_visit_text text2">
								<h3 contenteditable="true">What Are You <span>Searching</span></h3>
								<h3 contenteditable="true">Are You A <span>Visitor!</span></h3>
								<p contenteditable="true"> <i>Proin gravida nibh vel velit auctor aliquet Aenean <br>sollicitudin, </i></p>
								<p contenteditable="true">Lorem quis bibendum auctor, nisi elit consequat ipsum, nec <br>sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate<br> cursus a sit amet mauris. </p>
								<a class="theam_btn border-color color" href="#" tabindex="0">Explore More</a>
							</div>
						</div>
						<div class="col-md-6 col-sm-6">
							<div class="city_visit_fig">
								<figure class="box">
									<div class="box-layer layer-1"></div>
									<div class="box-layer layer-2"></div>
									<div class="box-layer layer-3"></div>
									<img src="{{asset('theme/local/extra-images/visit-fig1.jpg')}}" alt="">
								</figure>
							</div>
						</div>
					</div>
				</div>
			</div>
			<!-- CITY VISIT WRAP END-->

			<!--CITY NEWS2 WRAP START-->
			<div class="city_news2_wrap">
				<div class="container">
					<!--SECTION HEADING START-->
					<div class="section_heading center">
						<span contenteditable="true">Goverment</span>
						<h2 contenteditable="true">Most Recent News</h2>
					</div>
					<!--SECTION HEADING END-->
					<div class="row">
						<div class="col-md-4 col-sm-6">
							<div class="city_news2_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/news-01.jpg')}}" alt="">
									<div class="city_news2_text">
										<h5 contenteditable="true">City in Running for Public Art Funding from Bloomberg</h5>
										<p contenteditable="true">velit auctor aliquet. Aenean sollicitudin lore uisbibendum auctor nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio</p>
										<a class="theam_btn border-color color" href="#">Read More</a>
									</div>
									<div class="city_blog_social social2">
										<a href="#" tabindex="0" contenteditable="true">Charity & Funding</a>
										<div class="city_blog_icon_list">
											<ul class="social_icon">
												<li><a href="#"><i class="fa fa-facebook"></i></a></li>
												<li><a href="#"><i class="fa fa-twitter"></i></a></li>
												<li><a href="#"><i class="fa fa-linkedin"></i></a></li>

											</ul>
											<a class="share_icon" href="#"><i class="fa icon-social"></i></a>
										</div>
									</div>
								</figure>
							</div>
						</div>
						<div class="col-md-4 col-sm-6">
							<div class="city_news2_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/news-02.jpg')}}" alt="">
									<div class="city_news2_text">
										<h5 contenteditable="true">City in Running for Public Art Funding from Bloomberg</h5>
										<p contenteditable="true">velit auctor aliquet. Aenean sollicitudin lore uisbibendum auctor nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio</p>
										<a class="theam_btn border-color color" href="#">Read More</a>
									</div>
									<div class="city_blog_social social2">
										<a href="#" tabindex="0" contenteditable="true">Charity & Funding</a>
										<div class="city_blog_icon_list">
											<ul class="social_icon">
												<li><a href="#"><i class="fa fa-facebook"></i></a></li>
												<li><a href="#"><i class="fa fa-twitter"></i></a></li>
												<li><a href="#"><i class="fa fa-linkedin"></i></a></li>

											</ul>
											<a class="share_icon" href="#"><i class="fa icon-social"></i></a>
										</div>
									</div>
								</figure>
							</div>
						</div>
						<div class="col-md-4">
							<div class="city_news2_fig">
								<figure class="overlay">
									<img src="{{asset('theme/local/extra-images/news-03.jpg')}}" alt="">
									<div class="city_news2_text">
										<h5 contenteditable="true">City in Running for Public Art Funding from Bloomberg</h5>
										<p contenteditable="true">velit auctor aliquet. Aenean sollicitudin lore uisbibendum auctor nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio</p>
										<a class="theam_btn border-color color" href="#">Read More</a>
									</div>
									<div class="city_blog_social social2">
										<a href="#" tabindex="0" contenteditable="true">Charity & Funding</a>
										<div class="city_blog_icon_list">
											<ul class="social_icon">
												<li><a href="#"><i class="fa fa-facebook"></i></a></li>
												<li><a href="#"><i class="fa fa-twitter"></i></a></li>
												<li><a href="#"><i class="fa fa-linkedin"></i></a></li>

											</ul>
											<a class="share_icon" href="#"><i class="fa icon-social"></i></a>
										</div>
									</div>
								</figure>
							</div>
						</div>
					</div>
				</div>
			</div>
			<!--CITY NEWS2 WRAP END-->

			<!--CITY REQUEST WRAP START-->

            @endsection
